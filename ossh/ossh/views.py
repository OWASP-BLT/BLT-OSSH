...
from ossh.models import OsshArticle, OsshCommunity, OsshDiscussionChannel
from ossh.utils import fetch_github_user_data, get_client_ip
...
# Base URL for BLT API - configure in settings
BLT_API_BASE_URL = getattr(settings, 'BLT_API_BASE_URL', 'http://localhost:8000/api/v1')
...
def repo_recommender(user_tags, language_weights):
    """
    Recommend repositories based on user's tags and language preferences.
    Uses the BLT REST API endpoint instead of direct database queries.
    """
    ...
    try:
        ...
        # Using tags and language filters
        tags_param = ",".join(tag_names) if tag_names else ""
        languages_param = ",".join(language_list) if language_list else ""
        ...
        if languages_param:
            params['language'] = languages_param
        ...
        if isinstance(repos_data, dict) and 'results' in repos_data:
            repos_list = repos_data['results']
        elif isinstance(repos_data, list):
            repos_list = repos_data
        else:
            repos_list = []

        # Process each repository and calculate relevance scores
        for repo_data in repos_list:
            repo_tags = repo_data.get('tags', [])
            if isinstance(repo_tags, list) and len(repo_tags) > 0 and isinstance(repo_tags[0], dict):
                repo_tag_names = [tag.get('name') for tag in repo_tags]
            else:
                repo_tag_names = repo_tags if isinstance(repo_tags, list) else []

            tag_score = sum(tag_weight_map.get(tag, 0) for tag in repo_tag_names)
            language_score = language_weights.get(repo_data.get('primary_language', ''), 0)

            relevance_score = tag_score + language_score

            if relevance_score > 0:
                matching_tags = [tag for tag in repo_tag_names if tag in tag_weight_map]
                matching_languages = (
                    [repo_data.get('primary_language')]
                    if repo_data.get('primary_language') in language_weights
                    else []
                )
        ...
        logger.error(f"Error calling BLT API for repo recommendations: {e}", exc_info=True)
        return []
    except (ValueError, KeyError) as e:
        logger.error(f"Error processing API response for repo recommendations: {e}", exc_info=True)
        return []
